...
def MakeInstallerAndroid(version, **kwargs):
    ...
    # Bundle the Panda3D Studio sample games, if the Studio app is present.
    studio_games = os.path.join("android", "studio", "games")
    if os.path.isdir(studio_games):
        games_dir = os.path.join("apkroot", "assets", "games")
        if not os.path.exists(games_dir):
            os.makedirs(games_dir, mode=0o755)
        for base in sorted(os.listdir(studio_games)):
            if base.endswith(".py"):
                shutil.copy(os.path.join(studio_games, base),
                            os.path.join(games_dir, base))

    # Make an empty res folder.  It's needed for the apk to be installable, apparently.
    oscmd("mkdir -p apkroot/res")

    # The Studio app icon (a PNG generated by android/studio/make_icon.py).
    studio_icon = os.path.join("android", "studio", "res",
                               "drawable-nodpi", "ic_launcher.png")
    if os.path.isfile(studio_icon):
        icon_dir = os.path.join("apkroot", "res", "drawable-nodpi")
        if not os.path.exists(icon_dir):
            os.makedirs(icon_dir, mode=0o755)
        shutil.copy(studio_icon, os.path.join(icon_dir, "ic_launcher.png"))

    # Now package up the application.  Prefer the Studio manifest (a single
    # "Panda3D Studio" app with an in-app editor) over the plain test-app
    # manifest, when it exists.
    manifest_src = os.path.join("android", "studio", "studio_manifest.xml")
    if not os.path.isfile(manifest_src):
        manifest_src = "panda/src/android/pview_manifest.xml"
    oscmd("cp %s apkroot/AndroidManifest.xml" % manifest_src)
