...
বিল্ড করা APK-তে দুটি অ্যাপ (launcher icon) থাকবে:

| অ্যাপ | কাজ |
|---|---|
| **Panda Viewer** | `.egg` / `.bam` 3D মডেল খোলে। কোনো ফাইল না দিলে ডিফল্ট মডেল দেখায়। |
| **Panda Python** | যেকোনো `.py` স্ক্রিপ্ট চালায়। কোনো ফাইল না দিলে APK-র ভেতরে বান্ডেল করা **ডেমো অ্যাপ** (`android/default_app.py`) চালায় — ঘুরন্ত Panda3D পান্ডা মডেল। |
...
APK-র "Panda Python" অ্যাপটি যেকোনো `.py` ফাইল খুলতে পারবে (ফাইল ম্যানেজার থেকে
"Open with" → Panda Python)। অথবা `adb` দিয়ে:
...
- The APK ships a Panda Viewer (opens .egg/.bam) and a Panda Python runner
  (opens .py; with no file it runs the bundled demo `android/default_app.py`).
